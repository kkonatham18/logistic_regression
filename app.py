...
@st.cache_resource
def load_model():
    model = joblib.load("G:\logistic regression\logistic_regression_model.pkl")
    return model

# Load the dataset to get feature names
@st.cache_resource
def load_data():
    df = pd.read_csv("G:\logistic regression\heart.csv.xls")
    return df
...
try:
    ...
    st.error(f"❌ Error: {e}")
    st.info("Make sure the model file is saved at: G:\logistic regression\logistic_regression_model.pkl")
